import { createForOfIteratorHelper as _createForOfIteratorHelper, objectSpread2 as _objectSpread2, typeof as _typeof, toConsumableArray as _toConsumableArray, slicedToArray as _slicedToArray } from './_virtual/_rollupPluginBabelHelpers.js';
import { KIND_ROOT, KIND_TEXT, KIND_FRAGMENT, ACTION_MOUNT, ACTION_UPDATE_TEXT, ACTION_INSERT_CHILD, ACTION_REMOVE_CHILD, KIND_COMPONENT, ACTION_UPDATE_PROPS } from './types.mjs';
...
function _updateText(text, newText, internals, rootInternals) {
  return perform(text, rootInternals, {
    remote: function remote(channel) {
      return channel(ACTION_UPDATE_TEXT, text.id, newText);
    },
...
function _updateProps(component, newProps, internals, rootInternals) {
  ...
  return perform(component, rootInternals, {
    ...
    local: function local() {
      var mergedExternalProps = _objectSpread2(_objectSpread2({}, currentExternalProps), newProps);

      internals.externalProps = strict ? Object.freeze(mergedExternalProps) : mergedExternalProps;
      internals.internalProps = _objectSpread2(_objectSpread2({}, internals.internalProps), normalizedNewProps);
...
function tryHotSwappingValues(currentValue, newValue) {
  ...
  if (_typeof(currentValue) === 'object' && currentValue != null && !isRemoteFragment(currentValue)) {
...
function _appendChild(container, child, internals, rootInternals) {
  ...
  return perform(container, rootInternals, {
    remote: function remote(channel) {
      return channel(ACTION_INSERT_CHILD, container.id, container.children.length, serializeChild(child));
    },
    local: function local() {
...
function _removeChild(container, child, internals, rootInternals) {
  ...
  return perform(container, rootInternals, {
    remote: function remote(channel) {
      return channel(ACTION_REMOVE_CHILD, container.id, container.children.indexOf(child));
    },
    local: function local() {
...
function _insertChildBefore(container, child, before, internals, rootInternals) {
  ...
  return perform(container, rootInternals, {
    remote: function remote(channel) {
      return channel(ACTION_INSERT_CHILD, container.id, container.children.indexOf(before), serializeChild(child));
    },
    local: function local() {
      moveNodeToContainer(container, child, rootInternals);
...
function moveNodeToContainer(container, node, rootInternals) {
  ...
  var newTop = container.kind === KIND_ROOT ? container : tops.get(container);
